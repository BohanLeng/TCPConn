...
class PyTCPClient(tcp.TCPClientMsg):
    """
    ...
    Child classes can override these hooks:
      ...
      - on_message(self, msg: tcp.TCPMsg)
    ...
    def __init__(self):
        ...
        self.last_msg: Optional[tcp.TCPMsg] = None
    ...
    def OnMessage(self, msg: tcp.TCPMsg):
        self.last_msg = msg
    ...
    def on_message(self, msg: tcp.TCPMsg):
    ...
    def _ensure_header_size(self, m: tcp.TCPMsg) -> None:
        # Ensure header.size matches header + body length
        m.header.size = m.full_size()

    def send_msg(self, m: tcp.TCPMsg) -> None:
        self._ensure_header_size(m)
        self.send(m)

    # text and byte sender
    def send_text(self, text: str, type: int = 1, encoding: str = "utf-8") -> tcp.TCPMsg:
        b = text.encode(encoding)
        return self.send_bytes(b, type)

    def send_bytes(self, b: bytes, type: int = 1) -> tcp.TCPMsg:
        m = tcp.TCPMsg()
        m.header.type = type
        m.body = list(b)
        self.send_msg(m)
    ...
    def wait_for_message(
        self,
        timeout: float,
        predicate: Optional[Callable[[tcp.TCPMsg], bool]] = None,
        sleep: float = 0.02,
    ) -> Optional[tcp.TCPMsg]:
        """
...
def example():
    ...
    if not client.connect_and_wait("127.0.0.1", 9000, timeout=5.0):
